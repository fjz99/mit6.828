...
// 给每个core都映射一个stack
// 注意启动stack和内核栈的区别，这里映射的是内核栈，启动栈仅仅是启动的时候使用，然后就会进入用户程序了
// 以后再进入内核就是使用内核栈了
static void mem_init_mp(void) {
...
void page_init(void) {
  ...
  // 其他CPU启动时，需要把代码装入到这里，此时就不能把它放到free list中
...
struct PageInfo *page_lookup(pde_t *pgdir, void *va, pte_t **pte_store) {
  ...
  if (pg == NULL || !(*pg & PTE_P)) return NULL;
...
void *mmio_map_region(physaddr_t pa, size_t size) {
  ...
  base += size;
  if (base >= MMIOLIM) panic("mmio_map_region overflow!");
