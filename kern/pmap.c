...
static char *_boot_next_free = NULL;
...
static void *boot_alloc(uint32_t n) {
  ...
  // 启动时的临时内存分配器很简单，先找到内核的elf文件的代码和数据都用不到的第一个位置，向page取整。
  // 理论上是这样。。不过我测试的时候，end的位置在kern_pgdir全局变量之前，导致分配的时候出现了错误，所以我们再跨过一个page。
  if (!nextfree) {
    extern char end[];
    nextfree = ROUNDUP((char *)end, PGSIZE);
    nextfree += PGSIZE;
    // debug("init next free %08x,kern pgdir addr %08x\n", nextfree,
    // &kern_pgdir);
  }
...
void mem_init(void) {
  ...
  // 映射从UVPT开始的4MB空间，到kern dir的物理地址处。
  // 即，这是映射了一级页表，这样的话，就会把以及页表的内容当成二级页表？？
...
void page_init(void) {
  ...
  for (i = 0; i < npages; i++) {
    pages[i].pp_ref = 0;
    pages[i].pp_link = page_free_list;  //头插法，倒着建立链表
  ...
  //从内核开始的一段不能用(0xf0100000开始),计算出内核大小+boot alloc大小即可
  // KERNBASE
...
pte_t *pgdir_walk(pde_t *pgdir, const void *va, int create) {
  ...
  if (!(*pgdir & PTE_P)) {
    ...
    *pgdir = page2pa(info) | PTE_P | PTE_W | PTE_U;
    ...
    // FIXME
    // 这里存在诡异的bug，memset为0了，return前也检查了，但是return后就不是0了！
    // for (int i = 0; i < 1024; ++i) {
    //   if (*((pte_t *)page2kva(info) + i))
    //     cprintf("%08x\n", *((pte_t *)page2kva(info) + i));
    // }
    // cprintf("alloc ok!\n");

    // pte_t *rt = ((pte_t *)page2kva(info) + PTX(va));
    // cprintf("pageinfo pa %08x, va %08x\n", *((pte_t *)page2kva(info) +
    // PTX(va)),
    //         *((pte_t *)(0xf00457ff)));
    // return rt;
  }
  // if (*((pte_t *)KADDR(PTE_ADDR(*pgdir)) + PTX(va)))
  //   cprintf(" !!! va %08x\n", *((pte_t *)(0xf0045802)));

  // KADDR返回值为void*，所以指针加法是按照字节单位的。。。
  // if (*((pte_t *)KADDR(PTE_ADDR(*pgdir)) + PTX(va))) {
  //   cprintf("rt %08x, ?? %08x ,rt va %08x\n",
  //           *((pte_t *)KADDR(PTE_ADDR(*pgdir)) + PTX(va)),
  //           *((pte_t *)(0xf0045800)),
  //           (pte_t *)KADDR(PTE_ADDR(*pgdir)) + PTX(va));
  //   cprintf("va %08x,PTX %08x\n",va,PTX(va));
  // }
