...
// 先fork，再设置子进程的状态
// 之前从来没用过args、argv，是直接设置为0了，现在我们要根据传入的参数设置初始的栈数据，从而完成args、argv的传递
int spawn(const char *prog, const char **argv) {
  ...
  if ((r = open(prog, O_RDONLY)) < 0) return r;
  fd = r;
  ...
  if ((r = init_stack(child, argv, &child_tf.tf_esp)) < 0) return r;
  ...
  if ((r = copy_shared_pages(child)) < 0) panic("copy_shared_pages: %e", r);

  // 这里是故意的，为了测试。。其实应该设置为0
...
int spawnl(const char *prog, const char *arg0, ...) {
  ...
  va_start(vl, arg0);
  unsigned i;
  for (i = 0; i < argc; i++) argv[i + 1] = va_arg(vl, const char *);
  va_end(vl);
...
static int init_stack(envid_t child, const char **argv, uintptr_t *init_esp) {
  ...
  string_size = 0;
  for (argc = 0; argv[argc] != 0; argc++) string_size += strlen(argv[argc]) + 1;
  ...
  *init_esp = UTEMP2USTACK(&argv_store[-2]);
  ...
  if ((r = sys_page_map(0, UTEMP, child, (void *)(USTACKTOP - PGSIZE),
                        PTE_P | PTE_U | PTE_W)) < 0)
    goto error;
  if ((r = sys_page_unmap(0, UTEMP)) < 0) goto error;
...
static int map_segment(envid_t child, uintptr_t va, size_t memsz, int fd,
  ...
  for (i = 0; i < memsz; i += PGSIZE) {
    if (i >= filesz) {
      ...
      if ((r = sys_page_alloc(child, (void *)(va + i), perm)) < 0) return r;
    } else {
      // from file
      if ((r = sys_page_alloc(0, UTEMP, PTE_P | PTE_U | PTE_W)) < 0) return r;
      if ((r = seek(fd, fileoffset + i)) < 0) return r;
      if ((r = readn(fd, UTEMP, MIN(PGSIZE, filesz - i))) < 0) return r;
...
static int copy_shared_pages(envid_t child) {
  ...
  for (size_t i = 0; i < USTACKTOP; i += PGSIZE) {
    ...
    if ((tbl & PTE_P) && (tbl & PTE_SHARE)) {
      // cprintf("copy page:[%08x,%08x), pte %08x\n", i, i + PGSIZE, tbl&PTE_SHARE);
      if ((r = sys_page_map(thisenv->env_id, (void *)i, child, (void *)i,
                            tbl & PTE_SYSCALL)) < 0)
