...
static inline int32_t syscall(int num, int check, uint32_t a1, uint32_t a2,
  ...
  asm volatile("int %1\n"
               : "=a"(ret)
               : "i"(T_SYSCALL), "a"(num), "d"(a1), "c"(a2), "b"(a3), "D"(a4),
                 "S"(a5)
               : "cc", "memory");

  if (check && ret > 0)
    panic("syscall %d,arg %08x,%08x,%08x,%08x,%08x returned %d (> 0)", num, a1,
          a2, a3, a4, a5, ret);
...
int sys_cgetc(void) { return syscall(SYS_cgetc, 0, 0, 0, 0, 0, 0); }
...
void sys_yield(void) { syscall(SYS_yield, 0, 0, 0, 0, 0, 0); }
...
int sys_env_set_pgfault_upcall(envid_t envid, void *upcall) {
  return syscall(SYS_env_set_pgfault_upcall, 1, envid, (uint32_t)upcall, 0, 0,
                 0);
}
