...
static ssize_t devfile_write(struct Fd *fd, const void *buf, size_t n) {
  ...
  int r;
  size_t bytes = MIN(n, sizeof(fsipcbuf.write.req_buf));
  fsipcbuf.write.req_fileid = fd->fd_file.id;
  fsipcbuf.write.req_n = bytes;
  memcpy(fsipcbuf.write.req_buf, buf, bytes);
...
static int devfile_stat(struct Fd *fd, struct Stat *st) {
  ...
  if ((r = fsipc(FSREQ_STAT, NULL)) < 0) return r;
