...
void check_super(void) {
  if (super->s_magic != FS_MAGIC) panic("bad file system magic number");

  if (super->s_nblocks > DISKSIZE / BLKSIZE) panic("file system is too large");
...
// bitmap自己的块、super block都会被初始化为占用
bool block_is_free(uint32_t blockno) {
  if (super == 0 || blockno >= super->s_nblocks) return 0;
  if (bitmap[blockno / 32] & (1 << (blockno % 32)))
    return 1;  // 从这里能看出JOS的bitmap是连续的块
  return 0;
}
...
// 返回block的块号，从super开始为1；0为引导扇区的块
// 块缓存会缓存所有的块，包括引导扇区的块
...
// ppdiskbno存放的是f->f_direct[]中的项，或者是an entry in the indirect block.
// 即这其实是根据块号找到对应的块指针
// 函数名都叫walk了，所以不会处理块没分配的情况，根本就不管！
static int file_block_walk(struct File *f, uint32_t filebno,
                           uint32_t **ppdiskbno, bool alloc) {
  // LAB 5: Your code here.
  assert(filebno <= (f->f_size + BLKSIZE - 1) / BLKSIZE);
...
int file_get_block(struct File *f, uint32_t filebno, char **blk) {
  // LAB 5: Your code here.
  if (filebno >= NDIRECT + NINDIRECT) return -E_INVAL;
  // cprintf("get block for file %s,bno %d\n", f->f_name, filebno);
  int r;
  uint32_t *dst;
  if ((r = file_block_walk(f, filebno, &dst, true)) < 0) return r;
  if (!*dst) {
    r = alloc_block();
    if (r < 0) return r;
    *dst = r;
  }
  *blk = (char *)diskaddr(*dst);
  return 0;
}
...
static int dir_lookup(struct File *dir, const char *name, struct File **file) {
  ...
  for (i = 0; i < nblock; i++) {
    // 查找每个数据块，然后转换为File结构，即dir的数据块存放的是file结构
...
// 对路径字符串进行解析
static int walk_path(const char *path, struct File **pdir, struct File **pf,
                     char *lastelem) {
  const char *p;
...
int file_create(const char *path, struct File **pf) {
  ...
  if ((r = dir_alloc_file(dir, &f)) < 0) return r;
...
// offset和cnt看起来是文件的offset和cnt。。写入的就是buf全体
int file_write(struct File *f, const void *buf, size_t count, off_t offset) {
  ...
  if (offset + count > f->f_size)
    if ((r = file_set_size(f, offset + count)) < 0) return r;
...
int file_set_size(struct File *f, off_t newsize) {
  if (f->f_size > newsize) file_truncate_blocks(f, newsize);
  f->f_size = newsize;
  flush_block(f);
  return 0;
}
...
void file_flush(struct File *f) {
  ...
  for (i = 0; i < (f->f_size + BLKSIZE - 1) / BLKSIZE; i++) {
    if (file_block_walk(f, i, &pdiskbno, 0) < 0 || pdiskbno == NULL ||
        *pdiskbno == 0)
      continue;
    flush_block(diskaddr(*pdiskbno));
  }
  flush_block(f);  // 刷新dir的数据块中存放文件f元数据的块
  if (f->f_indirect) flush_block(diskaddr(f->f_indirect));
}

// Sync the entire file system.  A big hammer.
void fs_sync(void) {
  int i;
  for (i = 1; i < super->s_nblocks; i++) flush_block(diskaddr(i));
}
