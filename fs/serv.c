...
//    environments that have the file open. 类似于linux的的File结构。
...
//    file IDs to struct OpenFile.类似于dentry？

struct OpenFile {
  uint32_t o_fileid;    // file id，这里的是全局id
  struct File *o_file;  // mapped descriptor for open file
  int o_mode;           // open mode
  struct Fd *o_fd;  // Fd page，Fd有一个page大；Fd中的id是进程局部的id
};
...
// 一共最多有MAXOPEN个文件，如果多次打开同一个文件的话，会复用这个结构
  ...
  for (i = 0; i < MAXOPEN; i++) {
    switch (pageref(opentab[i].o_fd)) {
      case 0:
        ...
        /* fall through */
      case 1:
        // 等于1说明页面没有共享，是被我独占的，那么就说明了这个不是open函数处理过的（或者对应进程已经终止）
        // +MAXOPEN，来表示这个被引用了多次
...
int openfile_lookup(envid_t envid, uint32_t fileid, struct OpenFile **po) {
  ...
  if (pageref(o->o_fd) <= 1 || o->o_fileid != fileid) return -E_INVAL;
  *po = o;
  return 0;
}
...
int serve_open(envid_t envid, struct Fsreq_open *req, void **pg_store,
               int *perm_store) {
  ...
  if ((r = openfile_alloc(&o)) < 0) {
    if (debug) cprintf("openfile_alloc failed: %e", r);
    return r;
  }
  fileid = r;
  ...
  if (req->req_omode & O_CREAT) {
    if ((r = file_create(path, &f)) < 0) {
      if (!(req->req_omode & O_EXCL) && r == -E_FILE_EXISTS) goto try_open;
      if (debug) cprintf("file_create failed: %e", r);
      return r;
    }
  } else {
  try_open:
    if ((r = file_open(path, &f)) < 0) {
      if (debug) cprintf("file_open failed: %e", r);
      return r;
    }
  }
  ...
  if (req->req_omode & O_TRUNC) {
    if ((r = file_set_size(f, 0)) < 0) {
      if (debug) cprintf("file_set_size failed: %e", r);
      return r;
    }
  }
  if ((r = file_open(path, &f)) < 0) {
  ...
  if (debug) cprintf("sending success, page %08x\n", (uintptr_t)o->o_fd);
...
int serve_read(envid_t envid, union Fsipc *ipc) {
  ...
  struct Fsret_read *ret =
      &ipc->readRet;  // 注意这里公用一个地址，但是我们先用req，再用resp就没事
